...
import { TokenPayload } from "next-auth";
...
export default withAuth(
  async function middleware(req) {
    ...
    try {
      ...
      console.log("token token :", session?.user?.role);
      ...
      const allowedPaths = ["hospital", "patient", "insurance"];
      ...
      if (!allowedPaths.includes(basePath) || session.user?.role !== basePath) {
        return NextResponse.redirect(new URL("/login", req.url));
      }
